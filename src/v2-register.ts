...
function readServiceRegistration(): ServiceRegistration | undefined {
  const candidates = [
...
type LegacyPart = { type?: string; text?: string; synthetic?: boolean; [k: string]: unknown };
type LegacyMessage = {
  info?: {
    role?: string;
    agent?: string;
...
type LegacyClient = Record<string, any>;

/** Map a V2 flat session message (discriminated by `type`) to the V1 `{ info, parts }` shape. */
function toLegacyMessage(m: any): LegacyMessage {
  const metadata = (m.metadata ?? {}) as Record<string, unknown>;
  const info: LegacyMessage["info"] = {
    id: m.id,
    parentID: m.parentID,
    agent: m.agent,
    mode: m.mode,
...
function createLegacyClient(ctx: V2Context): LegacyClient {
  ...
  return {
    ...
    session: {
      async messages({ path }: { path: { id: string } }) {
        const messages = (await ctx.session.context({ sessionID: path.id } as any)) as any[];
        return { data: messages.map(toLegacyMessage) };
      },
      async get({ path }: { path: { id: string } }) {
        return { data: await ctx.session.get({ sessionID: path.id } as any) };
      },
      async prompt({ path, body }: { path: { id: string }; body?: any }) {
        // V1 compaction injection: prompt with a synthetic part + noReply.
        // V2 equivalent: ctx.session.synthetic (no assistant generation).
        const text = String(body?.parts?.find((p: LegacyPart) => p?.type === "text")?.text ?? "");
        await (ctx.session as any).synthetic({ sessionID: path.id, text });
    ...
    provider: {
      async list() {
        ...
        if (connected.length === 0) {
          const result = (await (ctx.provider as any).list()) as { data?: any[] };
...
export async function registerMemPlugin(ctx: V2Context): Promise<() => void> {
  const directory = ctx.location.directory;
  const legacyCtx = { directory, client: createLegacyClient(ctx) } as any;

  const hooks = (await OpenCodeMemPlugin(legacyCtx)) as any;
  ...
  const agentSpec = agentCfg.agent?.[STRUCTURED_OUTPUT_AGENT] as Record<string, any> | undefined;
  ...
  if (v1MemoryTool) {
    await ctx.tool.transform((editor) => {
      editor.add({
        ...
        input: MEMORY_TOOL_INPUT as any,
  ...
  if (hooks["chat.message"]) {
    await ctx.session.hook("prompt", async (event: any) => {
      ...
      if (isStructuredSummaryPrompt(userMessage)) return;
      if (isInternalStructuredSession(event.sessionID)) return;
  ...
  if (hooks["chat.params"]) {
    await ctx.session.hook("context", async (event: any) => {
      ...
      try {
        await userPromptManager.setSessionPromptModel(
          event.sessionID,
  ...
  if (legacyEventHandler) {
    void (async () => {
      try {
        ...
        if (!controller.signal.aborted) log("event subscribe error", { error: String(error) });
      }
    })();
  }
...
function isStructuredSummaryPrompt(userMessage: string): boolean {
  return (
    userMessage.includes("# User Profile Analysis") ||
    (userMessage.includes("Analyze this conversation.") && userMessage.includes('type="skip"'))
  );
}

async function computeInjectionDecision(
  ctx: V2Context,
  sessionID: string,
  CONFIG: any
): Promise<boolean> {
  const messages = (await ctx.session.context({ sessionID } as any)) as any[];
  const isRealUser = (m: any) => m?.type === "user" && m?.metadata?.synthetic !== true;
...
async function buildMemoryContext(
  ...
  try {
    ...
    if (CONFIG.chatMessage.excludeCurrentSession) {
      candidates = candidates.filter((m: any) => m.metadata?.sessionID !== sessionID);
    }
    if (CONFIG.chatMessage.maxAgeDays) {
      const cutoff = Date.now() - CONFIG.chatMessage.maxAgeDays * 86400000;
      candidates = candidates.filter((m: any) => new Date(m.createdAt ?? 0).getTime() > cutoff);
    }
    ...
    const projectMemories = {
      results: selected.map((c: any) => ({ similarity: c.similarity, memory: c.memory })),
...
let applyConfigRef: ((cfg: { agent?: Record<string, unknown> }) => void) | undefined;
async function applyStructuredOutputAgentConfigRef(cfg: { agent?: Record<string, unknown> }) {
  if (!applyConfigRef) {
