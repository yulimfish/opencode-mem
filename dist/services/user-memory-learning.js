...
export function shouldRunAutomaticProfileCleanup(previousPromptCount, addedPromptCount, interval = CONFIG.userProfileAutoCleanupInterval) {
    if (!CONFIG.userProfileAutoCleanupEnabled || !Number.isInteger(interval) || interval <= 0) {
        return false;
    }
    return (Math.floor(previousPromptCount / interval) <
        Math.floor((previousPromptCount + addedPromptCount) / interval));
}
async function runAutomaticProfileCleanup(userId) {
    try {
        const profile = await userProfileManager.getActiveProfile(userId);
        if (!profile)
            return;
        const profileData = JSON.parse(profile.profileData);
        const itemCount = profileData.preferences.length + profileData.patterns.length + profileData.workflows.length;
        if (itemCount < 2)
            return;
        const { aiCleanupProfile } = await import("./user-profile/ai-cleanup.js");
        const result = await aiCleanupProfile(profileData);
        if (result.diff.merged.length === 0 && result.diff.removed.length === 0)
            return;
        const updated = await userProfileManager.updateProfile(profile.id, result.cleaned, 0, `Automatic AI cleanup: ${result.diff.merged.length} merged, ${result.diff.removed.length} removed`);
...
export async function performUserProfileLearning(ctx, directory) {
    ...
    try {
        ...
        if (existingProfile && CONFIG.userProfileValidationEnabled) {
            const profileData = JSON.parse(existingProfile.profileData);
            const { data: decayed } = userProfileManager.decayInMemory(profileData);
            for (const arr of [decayed.preferences, decayed.patterns, decayed.workflows]) {
            ...
            const topPrefs = sortProfileItems(decayed.preferences, "confidence").slice(0, 5);
            const topPats = sortProfileItems(decayed.patterns, "frequency").slice(0, 3);
            const hasValidator = topPrefs.length >= 5;
            if (hasValidator) {
                const allValidated = [...topPrefs, ...topPats];
                validationPrefKeys = allValidated.map((p) => `${p.category || "_"}||${(p.description || "").substring(0, 30)}`);
                log("user-profile-learning: validation enabled", {
                    topPrefs: topPrefs.map((p) => `[${p.category}] ${(p.description || "").substring(0, 30)}`),
                    topPats: topPats.map((p) => `[${p.category}] ${(p.description || "").substring(0, 30)}`),
                });
...
${allValidated.map((p, i) => `${i}. [${p.category || "_"}] ${(p.description || "").substring(0, 30)} (conf: ${Math.round((p.confidence || 0) * 100) / 100})`).join("\n")}
        ...
        if (existingProfile) {
            ...
            while (!success && retries <= MAX_RETRIES) {
                if (retries > 0) {
                    ...
                    if (!existingProfile)
                        break;
                    const retryProfileData = JSON.parse(existingProfile.profileData);
                    ...
                    updatedProfileData = await userProfileManager.mergeProfileData(decayedRetry, llmResult, undefined, existingProfile.id);
                ...
                let changeSummary = generateChangeSummary(JSON.parse(existingProfile.profileData), updatedProfileData);
                const validationSummary = await applyValidations(updatedProfileData, llmResult, existingProfile.id, validationPrefKeys);
                if (validationSummary) {
                    changeSummary = changeSummary + "; " + validationSummary;
                }
                success = await userProfileManager.updateProfile(existingProfile.id, updatedProfileData, prompts.length, changeSummary);
                if (!success) {
            ...
            await userProfileManager.createProfile(userId, tags.user.displayName || "Unknown", tags.user.userName || "unknown", tags.user.userEmail || "unknown", llmResult, prompts.length);
            await userPromptManager.markMultipleAsUserLearningCaptured(prompts.map((p) => p.id));
        }
...
function buildCategorySummary(profileData) {
    const parts = [];
    ...
    const catSection = catParts.length > 0
...
function buildUserAnalysisContext(prompts, existingProfile, validationPrompt) {
...
- "User writes code and tests it" (too generic — covers everything)`;
    ...
    const truncate = (s) => s.length > maxBytes
...
export function createUserProfileAnalysisSchema(z) {
    return z.object({
...
export function createUserProfileToolSchema(existingProfile) {
    return {
        type: "function",
        function: {
...
async function applyValidations(profileData, llmResult, profileId, prefKeys) {
    const validations = llmResult.validations;
    if (!validations?.length || !prefKeys?.length)
        return null;
    ...
    for (const v of validations) {
        ...
        if (!key)
            continue;
        const item = allItems.find((i) => `${i.category || "_"}||${(i.description || "").substring(0, 30)}` === key);
        if (!item) {
...
Use the update_user_profile tool to save the ${existingProfile ? "updated" : "new"} profile.`;
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error("user-profile-learning: timeout")), 120000)),
            ]);
            ...
            if (existingProfile) {
                ...
                const merged = await userProfileManager.mergeProfileData(existingData, rawData, undefined, existingProfile.id);
...
async function buildLearningPaths(prompts, profileId) {
    const promptTexts = prompts.map((p, i) => `${i + 1}. ${p.content}`).join("\n");
    const systemPrompt = "You are a learning path analyst. Identify causal chains across a user's prompts. Output valid JSON.";
...
If no clear chains, return { "paths": [] }.`;
        ...
        try {
            ...
            if (v2Client) {
                result = (await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error("learning paths: opencode timeout")), 120000)),
                ]));
            }
        }
        catch (e) {
    ...
    await userProfileManager.updateProfile(profileId, data, 0, `Updated learning paths: ${result.paths.map((p) => p.topic).join(", ")}`);
}
