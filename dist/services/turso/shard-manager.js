...
function getValidatedEmbeddingDimensions(dimensions = CONFIG.embeddingDimensions) {
...
export class TursoShardManager {
    ...
    async withScopeWriteLock(scope, scopeHash, fn) {
    ...
    async getActiveShard(scope, scopeHash) {
    ...
    async getAllShards(scope, scopeHash) {
        const metadataDb = await this.ensureInitialized();
        const rows = scopeHash === ""
    ...
    async createShard(scope, scopeHash, shardIndex) {
        const metadataDb = await this.ensureInitialized();
    ...
    async registerExistingShard(scope, scopeHash, shardIndex, dbPath, vectorCount, isActive) {
        assertSafeScopeHash(scopeHash);
        const metadataDb = await this.ensureInitialized();
      ...
      `, [scope, scopeHash, shardIndex, storedPath, vectorCount, isActive ? 1 : 0, now]);
        const row = await metadataDb.get(`SELECT * FROM shards WHERE scope = ? AND scope_hash = ? AND shard_index = ?`, [scope, scopeHash, shardIndex]);
        if (!row) {
            throw new Error(`Failed to register shard ${scope}/${scopeHash}#${shardIndex}`);
        }
        return this.rowToShardInfo(row);
    }
    async initShardDb(db, dimensions = CONFIG.embeddingDimensions, embeddingModel = CONFIG.embeddingModel) {
        ...
        const existing = new Set(pragmaRows.map((r) => String(r.name)));
    ...
    async hasMatchingEmbeddingDimensions(db, shard) {
    ...
    async syncShardVectorCount(shard) {
    ...
    async getWriteShard(scope, scopeHash) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (!(await this.isShardValid(shard))) {
                throw new Error(`Shard ${shard.scope}/${shard.scopeHash}#${shard.shardIndex} is incompatible or corrupt. ` +
    ...
    async markShardReadOnly(shardId) {
        const metadataDb = await this.ensureInitialized();
        await metadataDb.run(`UPDATE shards SET is_active = 0 WHERE id = ?`, [shardId]);
    }
    async incrementVectorCount(shardId) {
        const metadataDb = await this.ensureInitialized();
        await metadataDb.run(`UPDATE shards SET vector_count = vector_count + 1 WHERE id = ?`, [
            shardId,
        ]);
    }
    async decrementVectorCount(shardId) {
        const metadataDb = await this.ensureInitialized();
        await metadataDb.run(`UPDATE shards SET vector_count = vector_count - 1 WHERE id = ? AND vector_count > 0`, [shardId]);
    }
    async setVectorCount(shardId, count) {
    ...
    async getShardByPath(dbPath) {
        ...
        const row = await metadataDb.get(`SELECT * FROM shards WHERE db_path LIKE '%/' || ? ESCAPE '\\'`, [escaped]);
        if (!row)
            return null;
        return this.rowToShardInfo(row);
    }
    ...
    async archiveShard(shardId, reason, archivePathOverride) {
        const metadataDb = await this.ensureInitialized();
        const row = await metadataDb.get(`SELECT * FROM shards WHERE id = ?`, [shardId]);
        if (!row)
            return null;
        const fullPath = this.resolveStoredPath(String(row.db_path), String(row.scope));
        await tursoConnectionManager.closeConnection(fullPath);
        const archivePath = archivePathOverride ?? `${fullPath}.${reason}-${process.pid}-${Date.now()}.bak`;
     ...
     */
    async reassignShardScope(shardId, newScopeHash, newDbPath, vectorCount, isActive) {
